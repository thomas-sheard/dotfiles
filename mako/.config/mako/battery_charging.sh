...
if [ $BATTERY_CHARGING -eq 1 ]; then
  notify-send "charging" "connected to power" -u low && bash "$HOME/.config/mako/battery_notifications/battery_warning.sh" && echo "ftlog"
elif [ $BATTERY_CHARGING -eq 0 ]; then
  notify-send "discharging" "power is unplugged" -u normal && bash "$HOME/.config/mako/battery_notifications/battery_warning.sh"
fi
